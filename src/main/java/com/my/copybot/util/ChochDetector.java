...
public class ChochDetector {
    ...
    public static int detectChochDirection(TimeSeries series) {
            ...
            // Находим текущий минимум и максимум
            ...
            // Находим последний слом тренда (CHOCH)
            for (int i = currentIndex - 1; i >= 1; i--) {
                ...
                double currentBarHigh = series.getBar(i).getMaxPrice().doubleValue();
                double currentBarLow = series.getBar(i).getMinPrice().doubleValue();

                // Условие CHoCH вверх (смена тренда на восходящий)
                if (currentBarHigh > prevHigh && currentLow < prevLow) {
                    return 1;
                }

                // Условие CHoCH вниз (смена тренда на нисходящий)
                if (currentBarLow < prevLow && currentHigh > prevHigh) {
                    return -1;
                }
            }
